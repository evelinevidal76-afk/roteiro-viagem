...
function FlightLeg({
  index, total, flight, onRemove, onEdit
}: {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'stretch', marginBottom: 8 }}>
      ...
      {/* Card */}
      <div style={{ flex: 1, background: 'rgba(201,151,60,0.08)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', marginBottom: index < total - 1 ? 0 : 0 }}>
...
function FlightForm({
  title, num, setNum, date, setDate, onSearch, loading, error,
  manual, manualData, updateManual, onSave, onCancelManual
}: any) {
  return (
    <Card style={{ marginBottom: 16 }}>
      ...
      {!manual ? (
        ...
        <>
          <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 12 }}>Voo não encontrado automaticamente. Preencha os dados:</div>
...
export default function StepFlight({ data, update, onNext }: Props) {
  ...
  const saveLeg = (flight: FlightInfo) => {
    if (editIndex !== null) {
      const updated = [...legs]; updated[editIndex] = flight; setLegs(updated)
      setEditIndex(null)
    } else {
      setLegs(prev => [...prev, flight])
    }
    setAdding(false); resetForm()
    // Update parent with first leg as outboundFlight and store all legs in notes-like field
    const all = editIndex !== null ? (() => { const u = [...legs]; u[editIndex] = flight; return u })() : [...legs, flight]
    update({ outboundFlight: all[0] })
  }
  ...
  const startEdit = (i: number) => {
    ...
    setManualData(leg); setManual(true)
    setAdding(true)
  }
  ...
  const handleSearchReturn = async () => {
    ...
    try {
      ...
      update({ returnFlight: emptyManual(returnNum, returnDate) })
    } finally { setLoadingReturn(false) }
  }

  const allLegs = legs
  const destination = allLegs.length > 0 ? allLegs[allLegs.length - 1].destinationCode : null
  const canContinue = allLegs.length > 0

  return (
    <div className="fade-up">
      <StepHeader
        title="Seus voos ✈️"
        subtitle="Adicione todos os voos da ida em sequência — incluindo conexões e escalas com voos diferentes."
      />
      ...
      {allLegs.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          {allLegs.map((leg, i) => (
            <FlightLeg key={i} index={i} total={allLegs.length} flight={leg}
      ...
      {adding && (
        <FlightForm
          title={editIndex !== null ? `Editar voo ${editIndex + 1}` : allLegs.length === 0 ? 'Primeiro voo da ida' : `Adicionar voo ${allLegs.length + 1}`}
      ...
      <div style={{ marginTop: 8, paddingTop: 20, borderTop: '1px solid var(--border)' }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 16 }}>
          <input type="checkbox" checked={data.hasReturn} onChange={e => update({ hasReturn: e.target.checked })}
        ...
        {data.hasReturn && (
          <div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
              <div style={{ flex: 2 }}>
                <label style={{ display: 'block', fontSize: 12, color: 'var(--muted)', marginBottom: 5 }}>Número do voo de retorno</label>
                <input type="text" value={returnNum} onChange={e => setReturnNum(e.target.value.toUpperCase())} placeholder="Ex: CM735"
                  style={{ width: '100%', padding: '10px 12px', background: 'var(--navy-soft)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--cream)', fontSize: 14 }} />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', fontSize: 12, color: 'var(--muted)', marginBottom: 5 }}>Data</label>
            ...
            <Button onClick={handleSearchReturn} loading={loadingReturn} variant="secondary" disabled={!returnNum || !returnDate}>
              🔍 Buscar voo de retorno
            </Button>
            {data.returnFlight && (
              <Card style={{ marginTop: 12 }}>
                <div style={{ fontSize: 11, color: 'var(--gold)', fontWeight: 600, marginBottom: 4 }}>✓ Retorno adicionado</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--cream)', fontFamily: 'var(--font-display)' }}>
                  {data.returnFlight.originCode || '?'} → {data.returnFlight.destinationCode || '?'}
                </div>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 3 }}>{data.returnFlight.flightNumber} · {data.returnFlight.date}</div>
              </Card>
            )}
          </div>
        )}
